# TestTheSpireTemplate

Chinese: [README.md](README.md)

TestTheSpireTemplate is a Slay the Spire 2 character mod template. It keeps the Godot mod project, C# code, BaseLib, MinionLib, a TestTheSpire test project, local API references, and skills in one repository. Developers can start character and card design from that project shape, and the AI can enter the work through the local source, tests, and skill instructions already present in the repo.

After a developer gives the AI card text, the STS2 path, and expected test coverage, the AI can inspect `local/sts2-decompiled/`, `local/BaseLib-StS2/`, `local/MinionLib/`, and nearby examples before changing code, localization, and tests. Tests run through MSBuild targets that launch STS2 in the headless TestTheSpire flow, so review output can point to the exact card and combat path that failed.

## Problem

STS2 mod work often gets stuck on manual testing and hard-to-reach reference material. A prompt such as "add a damage card" still leaves the AI needing the project base card class, card pool registration, dynamic variable style, BaseLib and MinionLib component APIs, localization keys, portrait paths, the test entry point, and the local STS2 install path. When one of those inputs is missing, the AI tends to guess an API or spend a long time manually decompiling assemblies to recover interface details. The result may compile while still giving no evidence that the in-game behavior is correct, so the developer has to test by hand, report failures, and ask for another round of changes.
...
```bash
git clone --recurse-submodules https://github.com/shadowversebydmod/TestTheSpireTemplate.git
cd TestTheSpireTemplate

dotnet new install .
dotnet new testthespire-sts2-character \
  -n MyFirstMod \
  -o ../MyFirstMod \
  --ModAuthor "Your Name"
```

Initialize submodules in the new project. `local/BaseLib-StS2`, `local/MinionLib`, and `bundled_skills` all come from `.gitmodules`. The `dotnet new` output already contains `.agents/skills`; after submodule initialization, the upstream skill source is visible as well:
...
```text
Please follow AGENTS.md and check whether this repository is ready for AI development. The STS2 path is `/path/to/SlayTheSpire2`. Confirm submodules, local/sts2-decompiled, BaseLib, MinionLib, bundled skills, and the TestTheSpire smoke test before implementing anything.
```
...
Please inspect nearby Attack examples, CharModCard, local/sts2-decompiled, local/BaseLib-StS2, and local/MinionLib before implementing, then run the focused test.
```

For BaseLib or MinionLib components, spell out the trigger:
...
- `.template.config/`: dotnet template metadata for `dotnet new testthespire-sts2-character`.
...
- `local/BaseLib-StS2/`: BaseLib source submodule matching the NuGet dependency.
...
This template references and extends the project structure from Alchyr's [ModTemplate-StS2](https://github.com/Alchyr/ModTemplate-StS2). BaseLib, MinionLib, and the existing STS2 mod template practices provide the starting shape; TestTheSpireTemplate adds the TestTheSpire test project, local source reference directories, and AI-facing skill entries on top of that foundation.
